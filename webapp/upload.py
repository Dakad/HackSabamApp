...
class UploadHandler(tornado.web.RequestHandler):
    ...
    def initialize(self, db, upload_folder,task_folder ):
        self._db = db
        self._upload_folder = upload_folder
        self._task_folder = task_folder
    ...
    def post(self):
        try:
            ...
            for idx, upload in enumerate(uploads):
                ...
                upload_f_name = os.path.join(self._upload_folder, upload_name)
                with open(upload_f_name, 'wb') as upload_file:
                    upload_file.write(upload['body'])

            # Write a task file for the pipeline
            new_task = os.path.join(self._task_folder, upload_id)
